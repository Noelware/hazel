...
$OtherJavaOpts = [Environment]::GetEnvironmentVariable("HAZEL_JAVA_OPTS") ?? "";
if ($OtherJavaOpts.Length -ne 0) {
    $SplitOptions = $OtherJavaOpts.Split(" ");
    foreach ($obj in $SplitOptions) {
        $ResolvedJavaOpts.Add($obj);
    }
}
...
& "$JavaExec" "$JavaOpts" -cp "$Classpath" "-Dorg.noelware.hazel.distribution.type=$DistributionType" org.noelware.hazel.cli.CliMainKt $Arguments
exit $LASTEXITCODE
