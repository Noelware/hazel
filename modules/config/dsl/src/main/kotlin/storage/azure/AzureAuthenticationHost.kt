...
package org.noelware.hazel.configuration.kotlin.dsl.storage.azure
...
fun AzureAuthenticationHost.toAzureConnectionAuth(): AzureConnectionAuth = when (this) {
...
sealed class AzureAuthenticationHost(val authType: AzureAuthType) {
    @Suppress("unused") // magically force the compiler to use `SAS_TOKEN` as the default, but this is never used for (de)serializing
    constructor(): this(AzureAuthType.SAS_TOKEN)

    @Serializable
    class ConnectionStringAuthenticationHost(
    ...
    @Serializable
    class SasTokenAuthenticationHost(
        @Serializable(with = SecretStringSerializer::class)
        @SerialName("connection_string")
        val sasToken: String
